...
plt.scatter(x=X_train[:,0], y=X_train[:,1], c=y_train, cmap=cMap, label='Trenirani')
plt.scatter(x=X_test[:,0], y=X_test[:,1], c=y_test, cmap=cMap, marker='x', label='Testirani')
plt.legend()
...
plt.fill_between(xd, yd, ymax, color='tab:red', alpha=0.5)
plt.xlim(xmin, xmax)
plt.ylim(ymin, ymax)
plt.show()
...
plt.colorbar(label='Tocno/Netocno', ticks=np.linspace(0, 1, 2))
plt.show()